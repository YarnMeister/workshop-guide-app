...
app.get('/api/insights/price-trends', requireAuth, async (req, res) => {
  try {
    ...
    let queryText = `
      WITH recent_data AS (
        SELECT
          ...
          ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY price_search_sold)::numeric, 0)::integer as median_price,
          COUNT(*) as total_sales,
          active_month
        FROM property_sales
        WHERE price_search_sold > 0 
          AND price_search_sold BETWEEN 50000 AND 10000000
    `;
    ...
    queryText += `
      ...
      SELECT month, median_price, total_sales
      FROM recent_data
      ORDER BY month DESC
      LIMIT $${params.length + 1}
    `;
